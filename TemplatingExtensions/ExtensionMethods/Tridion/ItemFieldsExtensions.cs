...
{
    /// <summary>
    /// Accessors for fields of different types.
    /// 
    /// Basic rules for the methods below:
    ///     * accept null as instance argument
    ///     * scalar methods may return null/default
    ///     * vector methods always return a sequence (possibly empty)
    /// </summary>
    public static class ItemFieldsExtensions
    {
        ...
        public static IEnumerable<DateTime> GetDateValues(this ItemFields fields, string fieldName = "date")
        {
            return
                null != fields && fields.Contains(fieldName)
                    ? (fields[fieldName] as DateField).Values
                    : new DateTime[0];
        }
        ...
        static readonly IDictionary<Type, Func<ItemFields, string, string>> valueResolver =
            new Dictionary<Type, Func<ItemFields, string, string>> {
                ...
                { typeof(DateField), (fields, name) => fields.GetDateValues(name).FirstOrDefault().ToString("yyyy-MM-dd HH:mm:ss") }
            };
